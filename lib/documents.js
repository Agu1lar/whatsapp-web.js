...
function isPathInsideRoot(root, resolvedPath) {
    const rootNorm = normalizeRoot(root);
    const resolvedNorm = path.resolve(resolvedPath);

    if (process.platform === 'win32') {
        const rootLower = rootNorm.toLowerCase();
        const resolvedLower = resolvedNorm.toLowerCase();
        return (
    ...
    return (
        ...
        resolvedNorm.startsWith(`${rootNorm}${path.sep}`)
    );
}
...
function safeResolve(root, relativePath) {
    ...
    const rootNorm = normalizeRoot(root);
    const resolved = path.resolve(rootNorm, ...sanitized.split('/'));

    if (!isPathInsideRoot(rootNorm, resolved)) {
        console.warn(
            `Caminho bloqueado (fora de DOCS_ROOT): ${String(relativePath).slice(0, 120)}`,
        );
        return null;
    }
...
function parseSendFileDirective(text) {
    ...
    if (!relativePath) {
        console.warn(
            `Diretiva @@SEND@@ bloqueada (caminho inválido): ${rawPath.slice(0, 120)}`,
        );
        return {
...
function resolveDocument(root, catalog, relativePath) {
    ...
    const target = sanitizeRelativePath(
        String(relativePath).replace(/\\/g, '/').trim(),
    );
    if (!target) {
        console.warn(
            `Busca de arquivo bloqueada (caminho inválido): ${String(relativePath).slice(0, 120)}`,
        );
        return null;
    }
