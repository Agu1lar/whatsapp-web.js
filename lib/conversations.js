...
function conversationAgeMs(conversation) {
...
function isConversationExpired(conversation) {
    const ttl = config.conversationTtlMs;
    if (!ttl || ttl <= 0) {
        return false;
    }

    return conversationAgeMs(conversation) > ttl;
}

function pruneExpiredConversations(states) {
    const ttl = config.conversationTtlMs;
    if (!ttl || ttl <= 0) {
        return 0;
    }
    ...
    for (const key of Object.keys(states)) {
        if (isConversationExpired(states[key])) {
            delete states[key];
            removed += 1;
        }
    }
...
function dropExpiredConversation(states, key) {
    if (states[key] && isConversationExpired(states[key])) {
        delete states[key];
        return true;
    }
...
function getConversation(states, key) {
    dropExpiredConversation(states, key);
...
async function runConversationMaintenance() {
    return withConversationLock('__maintenance__', async () => {
        const states = loadAllStates();
        const removed = pruneExpiredConversations(states);
        if (removed > 0) {
            saveAllStates(states);
            console.log(`Conversas inativas removidas: ${removed}`);
        }
        return removed;
    });
}

function startConversationMaintenance() {
    if (!config.conversationTtlMs || config.conversationTtlMs <= 0) {
        return;
    }
    ...
    runConversationMaintenance().catch((err) => {
        console.warn('Erro na limpeza de conversas:', err.message);
    });
    ...
    }, config.conversationPruneIntervalMs);
